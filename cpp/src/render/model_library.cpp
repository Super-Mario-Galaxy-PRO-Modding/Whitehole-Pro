...
std::string ModelLibrary::archiveNameFor(std::string_view objectName) const {
    ...
    // variants only apply while the low-poly setting is on.
    if (lowPoly_) {
        if (archiveExists(candidate + "Low.arc")) {
            return candidate + "Low.arc";
        }
        if (archiveExists(candidate + "Middle.arc")) {
            return candidate + "Middle.arc";
        }
    }
    if (archiveExists(candidate + ".arc")) {
    ...
    if (candidate != objectName && archiveExists(std::string(objectName) + ".arc")) {
        return std::string(objectName) + ".arc";
    }
    return {};
}
